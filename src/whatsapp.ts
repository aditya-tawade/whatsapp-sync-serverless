...
function cleanStaleSessionLocks(dataPath: string) {
  try {
    const sessionPath = path.resolve(dataPath, "session");
    ...
    for (const dir of targetDirs) {
      ...
      try {
        ...
        for (const entry of entries) {
          if (entry.startsWith("Singleton") || entry === "DevToolsActivePort") {
...
export function initPersistentWhatsApp(options?: {
  onQR?: (qr: string) => void;
  onReady?: () => void;
  authPath?: string;
}): Client {
  const dataPath = options?.authPath || process.env.WWEBJS_AUTH_PATH || "./.wwebjs_auth";
  cleanStaleSessionLocks(dataPath);
...
async function resolveProfilePicUrl(
  ...
  return await (client as any).pupPage.evaluate(async (contactId: string) => {
    ...
    if (result === undefined) {
      try {
        if ((globalThis as any).WWebJS?.getChat) {
          const chat = await (globalThis as any).WWebJS.getChat(contactId);
          result = await attempt(chat);
        }
      } catch {}
    }
...
export async function downloadFile(
  ...
  if (!photoUrl) {
    try {
      photoUrl = (await client.getProfilePicUrl(whatsappId)) || null;
    } catch (e) {
